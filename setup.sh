...
host_specific() {
  if [[ $(uname -s) == "Linux" && $(uname -m) == "x86_64" ]]; then
    ...
    WASI_SDK_ARTIFACT_PATH="dist/wasi-sdk-29.0-x86_64-linux.tar.gz"
    WASMTIME="wasmtime"
    NODEJS="nodejs"
    CABAL="cabal"
    BINARYEN="binaryen"
  ...
  if [[ $(uname -s) == "Linux" && $(uname -m) == "aarch64" ]]; then
    ...
    WASI_SDK_ARTIFACT_PATH="dist/wasi-sdk-29.0-aarch64-linux.tar.gz"
  ...
  if [[ $(uname -s) == "Darwin" ]]; then
    if [[ "${NIX_SYSTEM:-}" == "x86_64-darwin" || $(uname -m) == "x86_64" ]]; then
      ...
      WASI_SDK_ARTIFACT_PATH="dist/wasi-sdk-29.0-arm64-macos.tar.gz"
