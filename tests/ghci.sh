...
if [[ $FIREFOX_PATH != "" ]]; then
  echo -e "$GHCI_SCRIPT" | wasm32-wasi-ghc --interactive -package ghc -package Cabal -fghci-browser -fghci-browser-puppeteer-launch-opts="{\"browser\":\"firefox\",\"executablePath\":\"$FIREFOX_PATH\"}"
fi
...
if [[ $CHROME_PATH != "" ]]; then
  echo -e "$GHCI_SCRIPT" | wasm32-wasi-ghc --interactive -package ghc -package Cabal -fghci-browser -fghci-browser-puppeteer-launch-opts="{\"browser\":\"chrome\",\"executablePath\":\"$CHROME_PATH\",\"args\":[\"--no-sandbox\"]}"
fi
