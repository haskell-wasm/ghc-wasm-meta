...
pushd "$(mktemp -d)"
git clone --branch=v2.8.0 --depth=1 --recurse-submodules --shallow-submodules --jobs=2 https://github.com/agda/agda.git .
...
echo "package Agda" >> cabal.project.local
echo "  flags: +use-xdg-data-home" >> cabal.project.local
